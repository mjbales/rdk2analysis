...
void calcResultsStats(TString firstFileName,int numFiles,bool homogeneous,bool fourBody)
{
    ...
    if(fourBody)
        resultChain->SetBranchAddress("hitcodeg",&hitcodeg);
    ...
    for(int i=0;i<numEntries;i++)
    {
        ...
        {
            if(eeBS > 25)
                hitcodeeCount[hitcodee]+=1;
            if(fourBody)
                hitcodegCount[hitcodeg]+=1;
            numEvents+=1;
    ...
    for(int k=0;k<=MAXHITCODE;k++)
    {
        ...
        if(fourBody)
            statFile << "\t" << ((double) hitcodegCount[k])/((double) numEvents);
        statFile << endl;
    }
    statFile << endl;
...
void combineGammaEnergyHists()
{
    ...
    *geantHist=*geant_bareLow_hist + *geant_bareHigh_hist;
    ...
    benHist->Draw("same");
    tomHist->Draw("same");
    TLegend *l = new TLegend(0.5525025,0.672956,0.8421001,0.8449686,NULL,"brNDC");
...
TCanvas* makeRadRangeComparison()
{
    ...
    TLegend *l = new TLegend(0.5525025,0.672956,0.9421001,0.9449686,NULL,"brNDC");
...
//    double maximum=theHists[0]->GetMaximum();
   //double maximum=10000;
    ...
    for(int i=1;i<numHists;i++)
    {
        ...
        if(theLegend!=NULL)
            theLegend->AddEntry(theHists[i],theHists[i]->GetName(),"PL");
...
void makeHistWithGeantandMRK(TString mrkFileName,TString geantProtonFileName,TString geantElectronFileName,TString littlebFileName, TString eventFileName,unsigned int numFiles)
{
    ...
    saveFile.cd();
    TH3I* mrkP3DHist=new TH3I("mrkP3DHist","MRK Monte Carlo for Proton",
                              numBinsToF,protonToFBinMin,protonToFBinMax,
                              numBinsKE,protonKEBinMin,protonKEBinMax,
                              numBinsKE0,protonKE0BinMin,protonKE0BinMax);
    TH3I* mrkE3DHist=new TH3I("mrkE3DHist","MRK Monte Carlo for Electron",
                              numBinsToF,electronToFBinMin,electronToFBinMax,
                              numBinsKE,electronKEBinMin,electronKEBinMax,
                              numBinsKE0,electronKE0BinMin,electronKE0BinMax);
    TH3I* geantP3DHist=new TH3I("geantP3DHist","Geant4 Monte Carlo for Proton",
                              numBinsToF,protonToFBinMin,protonToFBinMax,
                              numBinsKE,protonKEBinMin,protonKEBinMax,
                              numBinsKE0,protonKE0BinMin,protonKE0BinMax);
    TH3I* geantE3DHist=new TH3I("geantE3DHist","Geant4 Monte Carlo for Electron",
                              numBinsToF,electronToFBinMin,electronToFBinMax,
                              numBinsKE,electronKEBinMin,electronKEBinMax,
                              numBinsKE0,electronKE0BinMin,electronKE0BinMax);
...
void makeMRKAndGeantHistograms(TString axis,ParType parType)
{
    ...
    TH3I* geant3DHistNew=(TH3I*) histFileNew.Get(geantHistString);
        ...
        TLegend* l = new TLegend(0.6620603,0.708042,0.8505025,0.8391608,NULL,"brNDC");
...
void checkGeantRKAccuracyToStandard(TString standardFileName,TString testFileName,int numEvents,double accuracyReq)
{
    ...
    for(int i=0;i<numEvents;i++)
    {
        ...
        if(standardFluxTreeEnd && testFluxTreeEnd)
            agreementCount++;
...
TCanvas* plotBNLSpectra500(int num)
{
   ...
   // theHists[16]=(TH1I*) f->Get("apr11_10_14");
    ...
    TLegend* theLegend = new TLegend(0.6620603,0.708042,0.8505025,0.8391608,NULL,"brNDC");
...
TCanvas* ploteeFVsExp()
{
    ...
    {
        if(theHists[1]->GetBinContent(i) <= 1)
            theHists[1]->SetBinContent(i,1);
        if(theHists[0]->GetBinContent(i) <= 1)
            theHists[0]->SetBinContent(i,1);
    }
...
void checkFalseTriple(TString detType,const int numDets)
{
    ...
    {
        if(j%1000000 == 0)
            cout << "File " << j/1000000  << " started!" << endl;
        ...
        if(lilbcut==0)
        {
            ...
            else
            {
                for(int i=0;i<numDets;i++)
                {
                    ...
                    if(GDetEDepE[i]+GDetEDepP[i]>9.5)
                        falseTripleDetCount[i]++;
                }
            }
        }
    }
...
void compareProtonSpectrum(TString geantResultsFile,TString inpStrimString,bool useLog)
{
    ...
    cout << "SRIM Total: " << total <<"  Avg: "<< average <<" kev" << endl;
     ...
     TLegend* theLegend = new TLegend(0.45,0.70,0.65,0.85,NULL,"brNDC");
...
void compareTestEm5(TString particle,TString material,int ver1,int ver2,TString histName,bool useLog)
{
    ...
    theHists[1]->Rebin(2);
    if(useLog)
        theHists[0]->GetYaxis()->SetRangeUser(1.e-5,1.);

    TLegend* theLegend = new TLegend(0.25,0.70,0.45,0.85,NULL,"brNDC");
    ...
    for (int i=0;i<resultsChain->GetEntries();i++)
    {
        ...
        if(dwcutb20==0)
        {
            ...
            if(hitcodee==HIT_DETECTOR && hitcodep==HIT_DETECTOR)
            {
                ...
                if(eef > 50 && eef < 500 && epf > 7 && epf < 31)
                    mcToFHist->Fill(tofp);
            }
        }
...
void makeMultiplicityFile(TString firstGammaFile, int numEPGFiles) //includes Kevin's Energy Resolution
{
    ...
    for (int k=0;k<numEPGFiles;k++ )
    {
        ...
        for(int j=0;j<resultsTree->GetEntries();j++)
        {
            ...
            {
                if(BGOEDepG[i]>0.1)
                    BGOEDepG[i]=applyEDepModel(BGOEDepG[i],i+1,rootRanGen);
...
void makeSingleBGOSpectrum(TString filePath,TString histName, bool useKevinResolution)
{
    ...
    {
        g->GetEntry(i);
        if(useKevinResolution)
            BGO1EDepTotal=applyEDepModel(BGO1EDepTotal,1,rootRanGen);
...
//        int numEnergyViolators=0;
        ...
        for (int j=0;j<resultsGammaChain->GetEntries();j++)
        {
            ...
            if(SBDEDepE>0)
                SBDEDepE+=rootRanGen->Gaus(0.0, SBD_ENERGY_SIGMA);

            //First is it an EP hit
            if(!checkForEP ||
               (SBDToFP >  theCuts.ToFPLow && SBDToFP < theCuts.ToFPHigh
                && SBDEDepP > PROTON_MIN_ENERGY && SBDEDepP < PROTON_MAX_ENERGY
                && SBDEDepE > theCuts.EDepELow && SBDEDepE < theCuts.EDepEHigh))
            {
    ...
    if(eDepModel==MATT_EXP_EDEP)
        histFilePath+="_EXPEDEP";
    if(eDepModel==KEVIN_GAUS_EDEP_NOEP)
        histFilePath+="_NOEP";
    if(eDepModel==KE_FIRST_MODEL)
        histFilePath+="_KEFIRST";
    if(eDepModel==KE_FIRST_NOEP_MODEL)
        histFilePath+="_KEFIRSTNOEP";
    histFilePath+=".root";
    ...
    for (int runType=0;runType<2;runType++)
    {
        for (int i=0;i<numEnergies;i++)
        {
            ...
            for(int j=0;j<numEntries;j++)
            {
                ...
                if(abs(energies[i]-BGOEDepGTotal) < peakSensitivity)
                    peakCounts[runType][i]+=1.;
                if(BGOEDepGTotal> 5)
                    countsAbove5keV[runType][i]+=1.;
                if(BGOEDepGTotal> 201)
                    countsAbove200keV[runType][i]+=1.;
            }
            delete resultsGammaChain;
...
void plotQuadraticVsExpResponse()
{
    ...
    for (int i=0;i<numEnergies;i++)
    {
        quadraticLO[i]=applyEDepModel(energies[i],detectorNumber, NULL, KEVIN_GAUS_EDEP)/energies[i];
        exponentialLO[i]=applyEDepModel(energies[i],detectorNumber, NULL, MATT_EXP_EDEP)/energies[i];
    }
...
{
    HistDim inpDimY={0,0,0}; HistDim inpDimZ={0,0,0};
    return combineAndScaleRadSetsToHist(hmgSet,lowSet, midSet,  highSet, drawString, epCut, gCut, inpDimX, inpDimY,inpDimZ);
}
...
TH1D* createHistWBGOResolution(TH1D* inpHist, int detectorNumber,TRandom3* rootRanGen,EDepModel eDepModel)
{
    ...
    for (int i = 1;i<= inpHist->GetNbinsX();i++)
    {
        ...
        {
            outputEnergy=applyEDepModel(binCenterEnergy,detectorNumber,rootRanGen,eDepModel,NULL);
            if(outputEnergy > getHistExtentLow(outHist) && outputEnergy <  getHistExtentHigh(outHist))
                outHist->Fill(outputEnergy);
...
{
    TString HistsDir=HISTS_DIR;
    TH1* mcEDepGHist[2]={getTabSeperatedHist(HistsDir+"MC_Hist"+mcDateString+"_EDepG_BGO_EPG_Sum_"+fourBodySet->GetGResultsID()+".txt",BGO_DIM),
        getTabSeperatedHist(HistsDir+"MC_Hist130116_EDepG_BGO_EPG_Sum_RID113.txt",BGO_DIM)};
    TH1* mcEDepEHist[2]={getTabSeperatedHist(HistsDir+"MC_Hist"+mcDateString+"_EDepE_BGO_EPG_Sum_"+fourBodySet->GetGResultsID()+".txt",EDEPE_DIM),
        getTabSeperatedHist(HistsDir+"MC_Hist130116_EDepE_BGO_EPG_Sum_RID113.txt",EDEPE_DIM)};
    TH1* mcToFPHist[2]={getTabSeperatedHist(HistsDir+"MC_Hist"+mcDateString+"_ToFP_BGO_EPG_Sum_"+fourBodySet->GetGResultsID()+".txt",TOFP_DIM),
        getTabSeperatedHist(HistsDir+"MC_Hist130116_ToFP_BGO_EPG_Sum_RID113.txt",TOFP_DIM)};
...
//    expEDepGHist[2]=getBenCSVHist(HistsDir+"Ben_bgo_energy_12_10_23.csv",13, BGO_DIM);
    ...
    //old MC vs new mc
    plotExpVersusMCToImage(0,NULL,2,mcEDepGHist,"BGO Energy", "EDepG_Sum_MCComparison_"+mcDateString+".png");
    ...
    for (int i=startingEvent;i<startingEvent+numberEvents;i++)
    {
        ...
        for (int k=0;k<numBGOs;k++)
        {
            if(BGOEDepTotal[k]>0)
            {
                ...
                BGOEDepBlurEdge[k]=applyEDepModel(BGOEDepTotal[k],k+1,rootRanGen,KHODYUK_PARAM_MODEL_WGAUSS,NULL);
                ...
                BGOEDepBlurLOAllExpParam[k]=applyEDepModel(BGOEDepTotal[k],k+1,rootRanGen,ALLEXP_PARAM_MODEL,NULL);
                ...
                BGOEDepBlurLOAllExpParamLow[k]=applyEDepModel(BGOEDepTotal[k],k+1,rootRanGen,ALLEXP_PARAM_MODEL_LOW,NULL);
                ...
                BGOEDepBlurLOAllExpParamHigh[k]=applyEDepModel(BGOEDepTotal[k],k+1,rootRanGen,ALLEXP_PARAM_MODEL_HIGH,NULL);
                ...
                BGOEDepResOnly[k]=applyEDepModel(BGOEDepTotal[k],k+1,rootRanGen,EDEP_WRESONLY,NULL);
...
//    fourBodySet.makeDerivedResultsFiles(GAMMA);
    ...
    plotExpVersusMCToImage(0,NULL,1,mcHists,"Proton Time of Flight "+ rids[4]+" EP Rate: " + d2str(epRate),imageString);
    delete mcHists[0];
    ...
    plotExpVersusMCToImage(0,NULL,1,mcHists,"Electron Energy "+ rids[4]+" EP Rate: " + d2str(epRate),imageString);
    delete mcHists[0];
    ...
    plotExpVersusMCToImage(0,NULL,1,mcHists,"#frac{EPG}{EP} "+ rids[4]+" EP Rate: " + d2str(epgRate),imageString);
    delete mcHists[0];
    ...
    plotExpVersusMCToImage(0,NULL,1,mcHists,"#frac{EPG}{EP} "+ rids[4]+" EP Rate: " + d2str(epgBAPDRate),imageString);
...
//	expHistInitBAPD->Scale(1.+bapdCorrection);
	...
	TLatex Tla; Tla.SetTextFont(42); Tla.SetTextSize(.04);
	...
	TLatex Tl; Tl.SetTextFont(42); Tl.SetTextSize(.04);
...
//	hrBAPD->GetYaxis()->SetMoreLogLabels(1);
	...
	residualHistBAPD->Add(mcHistBAPD, -1);residualHistBAPD->Divide(mcHistBAPD);
...
void makeOtherParameterPlot(TString expID, double correction, int mcAIDNum, TString coTypeStr, TString plotType,TString imageType,bool halfPlotNoResid)
{
		...
		TCanvas* theCanvas;
		TPad* pad1=NULL; //resididual hist
		TPad* pad2=NULL;
		if(halfPlotNoResid)
		{
		...
		if(coTypeStr=="EPG" || coTypeStr=="EPBG")
			expCoTypeStr="epg";
		...
		if(coTypeStr == "EP")
			...
			titleString+=";APD ep#gamma counts";
