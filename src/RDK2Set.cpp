...
RDK2Set::RDK2Set(TString inpName, TString inpTitle,int inp_numFiles,TString inp_eventsDateString,TString inp_eventsTypeString,TString inp_pResultsID,
...
RDK2Set::~RDK2Set()
{
    ...
    if(pResultsChain==0)
        delete pResultsChain;

    if(eResultsChain==0)
        delete eResultsChain;

    if(gResultsChain==0)
        delete gResultsChain;
...
void RDK2Set::loadEventsChain()
{
    ...
    if(isEventGenerator)
    {
        ...
        if(eventsChain==0)
            eventsChain= makeEventChain(filePath.Data(),"t",numFiles,true);
    }
    else
    {
        ...
        if(eventsChain==0)
            eventsChain= makeEventChain(filePath.Data(),"t",numFiles);
    }
...
void RDK2Set::loadPResultsChain()
{
    ...
    if(pResultsChain==0)
    {
        pResultsChain = makeResultChain(filePath.Data(),"g",numFiles);
    }
}
...
void RDK2Set::loadEResultsChain()
{
    ...
    if(eResultsChain==0)
        eResultsChain = makeResultChain(filePath.Data(),"g",numFiles);
}
...
void RDK2Set::loadGResultsChain()
{
    ...
    if(gResultsChain==0)
        gResultsChain = makeResultChain(filePath.Data(),"g",numFiles);
}

void RDK2Set::loadLittleBChain()
{

    TString filePath=LITTLEB_DIR;


    filePath+="MJB-Evn-"+eventsTypeString+"-n1.B3-0-20"+eventsDateString+"_lilb.root";
    if(littleBChain == 0)
        littleBChain = makeEventChain(filePath.Data(),"lilbTree",numFiles);
}
...
void RDK2Set::loadDervChains()
{
    ...
    for (int i=0;i<3;i++)
    {
        ...
        if(!FileExists(derivedPath.Data()))
            makeDerivedResultsFiles(parType);
        if(i==0)
        {
...
{
    HistDim inpHistDimY={0,0,0}; HistDim inpHistDimZ={0,0,0};
    return makeHist( histName, drawString, inpCut, inpHistDimX,  inpHistDimY, inpHistDimZ);
}
...
double RDK2Set::makeAllStandardHists(TString nameString,double& error,RDK2CutSet inpCutSet,CoDet detType,double inpEPPer3Decay,double branchingRatio)
{
    ...
    TString coString;
    TH1* radAvgHist=NULL;
    TH1* radAvgVarHist=NULL;
    TH1* radDetHists[12]={NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL};
    TH1* radDetVarHists[12]={NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL};
    if(detType == DET_EP)
    {
        coString="EP";
    }
...
//        numDets=3;
    ...
    if(detType == DET_EPG || detType == DET_EPBG)
    {
        ...
        eEHist->Sumw2(); eEHist->Scale(scaleFactor/numDets);
        pEHist->Sumw2(); pEHist->Scale(scaleFactor/numDets);
        pTHist->Sumw2(); pTHist->Scale(scaleFactor/numDets);
        radAvgHist->Sumw2(); radAvgHist->Scale(scaleFactor/numDets);
        radAvgVarHist->Sumw2(); radAvgVarHist->Scale(scaleFactor/numDets);
        convertTH1ToTXT(radAvgHist,radAvgHist->GetTitle());
        convertTH1ToTXT(radAvgVarHist,radAvgVarHist->GetTitle());
        for (int i = 0;i< numDets;i++)
        {
        	radDetHists[i]->Sumw2(); radDetHists[i]->Scale(scaleFactor);
        	convertTH1ToTXT(radDetHists[i],radDetHists[i]->GetTitle());
            radDetVarHists[i]->Sumw2(); radDetVarHists[i]->Scale(scaleFactor);
